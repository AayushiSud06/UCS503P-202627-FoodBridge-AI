...
> Last verified against the repository: 2026-09-10, branch `master`, HEAD `2ce3d71`.
...
> accepting organisation exactly and a donor, a courier and a peer organisation not at all.
> `Donation.match_score` still stores the precise frozen decision (D-30) — only the
...
> **Task 29's landing-page cleanup (D-48) is committed as `6961555`, Task 30's
> login-page redesign (D-49) as `f863a94` and Task 31's availability fix (D-50) as
> `2ce3d71`, now HEAD.**
...
> correction as `9b11353`. ⚠️ **The working tree carries one uncommitted task — Task 32,
> a delivered run reaching the courier's history** — D-51: the desktop courier portal
> recorded a finished run only at `COMPLETED`, which is the **kitchen's** confirmation of
> receipt (`TRANSITION_ROLES[COMPLETED]` is `{ngo, admin}`). `DELIVERED` is as far as a
> courier can drive, so marking a run delivered erased it from every desktop courier surface
> until a kitchen confirmed. `COURIER_FINISHED` is now `['DELIVERED', 'COMPLETED']`, read by
> the history log and the tasks page's completed section. **Frontend only** — two screens in
> one folder plus one new test file, and one new backend test file that ruled the API out;
> no backend, lifecycle, schema, API, authorization or impact-arithmetic change.
> 283 backend and 106 frontend tests pass. See `TASKS.md` → *Current*. The lifecycle write-authorization work is committed — D-34 as
...
> *Backlog → A* rather than treated as closed. **`HA-3b` is closed by Task 28
> (D-47, `d611424`); `HA-3a` remains open** and its control is abuse-limiting on donation
