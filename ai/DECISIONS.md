...
## D-21 · `create_all` at startup instead of migrations **[documented]**

**Decision.** `main.py`'s lifespan calls `Base.metadata.create_all`; `cli._session()`
does the same so bootstrapping works on a bare checkout.

**Reasoning** (`main.py`): "Fine for SQLite and coursework. Introduce Alembic before the
schema has to change without dropping data." An explicitly acknowledged temporary
measure, not an oversight.

**Constraints.** ⚠️ Creates missing tables but **never alters existing ones** — a new
model column silently does not appear and queries touching it fail. No history, no
rollback. Current workaround is deleting the database file. **The project's own comment
marks this as the thing to fix first.**
