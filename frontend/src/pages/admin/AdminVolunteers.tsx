...
/** Statuses in which a courier is still holding a load. */
...
export default function AdminVolunteers() {
  ...
  // The API does not carry an "active trips" count, and it should not — it is
  // derivable from the donations a courier currently holds, and a stored
  // counter would be one more thing that can drift.
  const activeTrips = (volunteerId: string) =>
    donations.filter(d => d.volunteerId === volunteerId && IN_FLIGHT.includes(d.status)).length;
    ...
    <div className="space-y-6 max-w-7xl">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Courier availability, ratings and live load across the platform.
        </p>
      </div>

      {volunteers.length === 0 ? (
        <div className="card p-8 text-center text-gray-500 text-sm">
          No couriers have registered yet.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {volunteers.map(v => {
            const active = activeTrips(v.id);
            return (
              <div key={v.id} className="card p-5 space-y-3 hover:shadow-md transition-shadow">
                ...
                <div className="text-xs text-gray-600 border-t border-gray-100 pt-2 flex items-center justify-between gap-2">
                  ...
                  <span className="text-emerald-700 font-semibold flex items-center gap-0.5 text-[11px] shrink-0">
                    <ShieldCheck size={13} /> Courier
                  </span>
                </div>
              </div>
            );
